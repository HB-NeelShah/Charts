...
open class BarChartRenderer: BarLineScatterCandleBubbleRenderer
{
    ...
    @objc open func drawDataSet(context: CGContext, dataSet: IBarChartDataSet, index: Int)
    {
        ...
        for j in stride(from: 0, to: buffer.rects.count, by: 1)
        {
			...
			if dataSet.showCornerRadious == true
			{
				let clipPath = UIBezierPath(roundedRect: barRect, cornerRadius: CGFloat(dataSet.cornerRadiousValue)).cgPath
                        ...
                        else
                        {
                            ...
                            for k in 0 ..< transformed.count
                            {
                                ...
                                if dataSet.isDrawValuesEnabled
                                {
                                    ...
                                    if dashBorder && (k % dataSet.moduleToShowDash == 0){
                                ...
                                if let icon = e.icon, dataSet.isDrawIconsEnabled
                                {
                                    if dataSet.isToShowIconAtSpecific == true{
                                        if dataSet.barIndex == index && dataSet.barInnerIndex == k{
                                            ChartUtils.drawImage(
                                                context: context,
                                                image: icon,
                                                x: x + iconsOffset.x,
                                    ...
                                    else if dataSet.barInnerIndex > 0 && dataSet.barInnerIndex == k{
                                        let imageSecond = e.data as! UIImage
                                        ChartUtils.drawImage(
                                            context: context,
                                            image: imageSecond,
                                            x: x + iconsOffset.x,
